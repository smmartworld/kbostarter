        ...
        if status == '종료':
            ...
            for j, rot_row in rot_df.iterrows():
                ...
                with btn_cols[j]:
                    ...
                    is_absent = False
                    if pname in combined_absences and selected_dt < pd.to_datetime(combined_absences[pname]):
                        is_absent = True
                    
                    if is_absent:
                        # 휴식 중이면 텍스트를 바꾸고, 아래 st.button에서 disabled 처리할 거야
                        return_date_str = pd.to_datetime(combined_absences[pname]).strftime('%m/%d')
                        btn_text = f"🏥 휴식중\n{pname}\n({return_date_str} 복귀)"
